...
class RandomJitter:
    """ Generate perturbations by adding noise to xyz
    """
...
class RandomCrop:
    ...
    def __call__(self, sample):
        # sample['crop_proportion'] = self.p_keep # TODO
...
class RandomTransformSE3:
    def __init__(self, rot_mag: float = 180.0, trans_mag: float = 1.0,
                    random_mag: bool = False, xy_rot_scale: float = 1.0):
        ...
        Args:
            ...
            random_mag (bool): If true, will randomize the maximum rotation, i.e. will bias towards small
                               perturbations
            xy_rot_scale (float): rotation discount in XY axis (non-upward direction)
        """
        self._rot_mag = rot_mag
        self._trans_mag = trans_mag
        self._random_mag = random_mag
        self.xy_rot_scale = xy_rot_scale
    ...
    def apply_transform(self, p0, transform_mat):
        ...
        if p0.shape[1] > 6:
            ...
            p1 = np.concatenate((p1, p0[:, 3:]), axis=-1)
    ...
    def transform(self, pt):
        ...
        return self.apply_transform(pt, transform_mat)
    ...
    def __call__(self, sample):
        ...
        elif 'points_src' in sample:
            src_transformed, transform_r_s, transform_s_r = self.transform(sample['points_src'])
            sample['transform_gt'] = transform_r_s  # Apply to source to get reference cloud
...
class RandomTransformSE3_euler(RandomTransformSE3):
    ...
    def generate_transform(self):
        ...
        # Generate rotation, if xy_rot_scale = 0, only rotate around Z-axis
        anglex = np.random.uniform() * np.pi * rot_mag / 180.0 * self.xy_rot_scale
        angley = np.random.uniform() * np.pi * rot_mag / 180.0 * self.xy_rot_scale
        ...
        rand_SE3 = np.identity(4, dtype=np.float32)
        rand_SE3[:3, :3] = R_ab
        rand_SE3[:3, 3] = t_ab
        return rand_SE3
...
class RandomRotatorZ(RandomTransformSE3):
    ...
    """
    def __init__(self, rot_mag=360):
        super().__init__(rot_mag=rot_mag)

    def generate_transform(self):
        """ Generate a random SE3 transformation (4, 4)
        """
        rand_rot_deg = np.random.random() * self._rot_mag
        rand_rot = Rotation.from_euler('z', rand_rot_deg, degrees=True).as_matrix() # as_dcm
